        ...
        try:
            ...
            for entry in feed.entries[:limit]:
                articles.append({
                    'title': entry.get('title', ''),
                    'url': entry.get('link', ''),
                    'summary': entry.get('summary', '') or entry.get('description', '') or 'Financial Times报道',
                    'source': self.source,
                    'publish_time': datetime(*entry.published_parsed[:6]) if hasattr(entry, 'published_parsed') and entry.published_parsed else datetime.now(),
                    'created_at': datetime.now()
                })
...
class SinaFinanceScraper(EconomyScraper):
    ...
    def scrape_articles(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            # 更精确的选择器 - 新浪财经滚动新闻
            news_items = soup.select('.list_009 li a, .feed-card-title-link, .news-item a')[:100]
            ...
            for item in news_items:
                try:
                    ...
                    # 确保是新浪财经的文章链接
                    if url and url not in seen_urls and 'sina.com.cn' in url and len(url) < 500:
                        # 过滤非文章链接
                        if any(x in url for x in ['/video/', '/photo/', '/search', '.jpg', '.png', '/tag/', '/api/']):
                            continue

                        # 确保是财经相关的URL
                        if 'finance.sina.com.cn' in url or 'money.sina.com.cn' in url or 'stock.finance.sina.com.cn' in url:
                            seen_urls.add(url)
                ...
                try:
                    ...
                    for item in news_items:
                        ...
                        try:
                            ...
                            if 'finance.sina.com.cn' in url and not any(x in url for x in ['/video/', '/photo/', '.jpg']):
                                seen_urls.add(url)
                                articles.append({
                                    'title': title,
                                    'url': url,
        ...
        try:
            ...
            for item in news_items:
                try:
                    ...
                    # 去重并确保是有效文章链接
                    if url and url not in seen_urls and 'caixin.com' in url and len(url) < 450:
                        # 过滤非文章链接
                        if any(x in url for x in ['/video/', '/photo/', '/tag/', '/search', '#gocomment']):
                            continue
                        ...
                        # 提取图片
                        image_url = ""
                        img_tag = item.find_next('img') if hasattr(item, 'find_next') else None
                        if not img_tag:
                            parent = item.find_parent()
...
class WallStreetCNScraper(EconomyScraper):
    ...
    def scrape_articles(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            # 东方财富网的新闻链接
            news_items = soup.select('a[href*="finance.eastmoney.com"], .newsli a, .infotxt a')[:50]
            ...
            for item in news_items:
                try:
                    ...
                    if url and url not in seen_urls and 'eastmoney.com' in url and len(url) < 450:
...
class TencentStockScraper(EconomyScraper):
    ...
    def scrape_articles(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            # 新浪财经的新闻链接
            news_items = soup.select('a[href*="finance.sina.com"], .feed-card-item a, .news-item a')[:50]
            ...
            for item in news_items:
                try:
                    ...
                    if url and url not in seen_urls and 'sina.com' in url and len(url) < 450:
                        if any(x in url for x in ['/video/', '/photo/', '/search', '.jpg', '.png']):
                            continue
...
class PhoenixFinanceScraper(EconomyScraper):
    ...
    def scrape_articles(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            # 凤凰财经的新闻链接
            news_items = soup.select('a[href*="finance.ifeng.com"], .news-stream a, .news_item a, h3 a, h2 a')[:50]
            ...
            for item in news_items:
                try:
                    ...
                    if url and url not in seen_urls and 'ifeng.com' in url and len(url) < 450:
                        # 过滤非文章链接
                        if any(x in url for x in ['/video/', '/photo/', '/search', '.jpg', '.png', '/tag/']):
                            continue
                ...
                try:
                    ...
                    if url and not url.startswith('http'):
                        if url.startswith('//'):
                            ...
                            # 判断是哪个域名的路径
                            if url.startswith('/commonDetail/') or url.startswith('/news/'):
                    ...
                    if url and url not in seen_urls and 'cnstock.com' in url:
                        # 过滤非文章链接
                        if any(x in url for x in ['/video/', '/photo/', '/search', '.jpg', '.png', '/html/', '/about', '/cooperation']):
                            continue

                        # 确保是新闻文章URL（commonDetail是文章详情页）
                        if '/commonDetail/' in url or '/news/' in url or '/company/' in url or '/stock/' in url or url.count('/') >= 4:
                            seen_urls.add(url)
