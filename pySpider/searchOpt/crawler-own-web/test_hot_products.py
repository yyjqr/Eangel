...
def test_hot_products():
    ...
    # 2. 显示权重最高的产品新闻
    print("\n2. 权重最高的产品新闻 (前10条):")
    top_products = TechNews.objects.filter(
    ...
    if hot_products.exists():
        ...
        for hp in hot_products:
            ...
            print(f"  Rank {hp.rank}: [{hp.category}] {hp.title[:50]}... (权重: {hp.rate:.2f})")
    else:
        print("  暂无数据。请运行: python3 manage.py update_hot_products")

    # 4. 统计信息
    print("\n4. 统计信息:")
    print(f"  产品新闻总数: {TechNews.objects.filter(category__in=product_categories).count()}")
    print(f"  热点产品记录数: {HotProduct.objects.count()}")
    print(f"  最新更新时间: {HotProduct.objects.order_by('-updated_at').first().updated_at if HotProduct.objects.exists() else '无'}")
