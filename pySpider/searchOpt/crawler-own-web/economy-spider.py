...
# 读取配置
with open(os.path.join(os.path.dirname(__file__), '.', 'economy_key_config_map.json')) as cfg_f:
    cfg = json.load(cfg_f)
...
class NewsScraper:
    ...
    def calculate_weight(self, title):
        ...
        if MILITARY_SIGNAL_KEYWORDS:
            mil_score = sum(
                float(w) for k, w in MILITARY_SIGNAL_KEYWORDS.items()
    ...
    def determine_category(self, title, keywords):
        ...
        # 定义经济分类规则
        categories = {
            '宏观经济': ['gdp', 'inflation', 'fed', 'imf', 'policy', 'rate', 'tax', 'economy', 'macro', '央行', '通胀', '利率', '政策'],
            '股市投资': ['stock', 'market', 'nasdaq', 'dow', 'ipo', 'earnings', 'fund', 'invest', '股市', '上证', '深证', 'A股'],
            '企业动态': ['merger', 'acquisition', 'ceo', 'layoff', 'report', 'microsoft', 'apple', 'tesla', 'google', 'amazon', '并购', '财报', '企业', '营收', '利润'],
            '金融科技': ['fintech', 'crypto', 'bitcoin', 'blockchain', 'bank', 'payment', '金融科技', '区块链', '数字货币'],
            '贸易与供应链': ['trade', 'tariff', 'supply chain', 'freight', 'export', 'import', '贸易', '关税', '供应链'],
            '产业经济': ['industry', 'manufacturing', 'production', '产业', '制造业', '生产', 'chip industry', 'ev market', 'semiconductor industry', '半导体产业', '汽车产业', '新能源产业'],
            '经济评论': ['opinion', 'analysis', 'forecast', 'outlook', 'trend', '预测', '趋势', '分析']
        }
    ...
    def save_to_txt(self, articles, filename=OUTPUT_FILE):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            for idx, article in enumerate(articles, 1):
                ...
                if len(article) >= 8:
                    title, url, weight, image_url, publish_time, created_at = article[:6]
                elif len(article) == 6:
