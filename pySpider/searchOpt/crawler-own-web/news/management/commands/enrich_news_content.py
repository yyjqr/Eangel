...
ECONOMY_CATEGORIES = {'宏观经济', '经济综合', '经济评论', '股市投资', '金融科技',
                      '贸易与供应链', '企业动态', '产业经济'}
PRODUCT_CATEGORIES = {'机器人', '智能硬件', '汽车与自动驾驶', 'VR/AR/XR',
                      '产品发布', '航空航天', '军事', '军工与航天'}
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--use-crawl4ai', action='store_true', default=False,
            help='强制使用 crawl4ai（适合 JS 密集站，速度慢）'
        )
        parser.add_argument(
            '--workers', type=int, default=3,
            help='并发线程数（默认 3，避免过多同时请求）'
        )
        parser.add_argument(
            '--dry-run', action='store_true', default=False,
            help='仅打印需要增强的文章，不写数据库'
        )
        parser.add_argument(
            '--min-id', type=int, default=0,
            help='只处理 id >= 此值的文章（用于断点续传）'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"扫描 {checked} 条  →  需要增强: {len(to_enrich)} 篇"
        )
