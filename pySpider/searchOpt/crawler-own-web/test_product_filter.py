...
def test_product_news_filter():
    ...
    # 测试条件
    product_categories = ['机器人', '智能硬件', '汽车与自动驾驶', 'VR/AR/XR', '军事', '航空航天', '军工与航天', '产品发布', '芯片与半导体']
    ...
    # 2. 应用筛选条件
    print(f"\n2️⃣  应用筛选条件后：")
    filtered_news = TechNews.objects.filter(
    ...
    # 4. 检查是否有经济类文章混入
    print(f"\n4️⃣  检查经济类文章：")
    economic_categories = ['宏观经济', '经济综合', '经济评论', '股市投资', '金融科技', '贸易与供应链', '企业动态', '产业经济']
    economic_news = TechNews.objects.filter(
        category__in=product_categories,
        rate__gt=2.0
    ).filter(
        Q(category__icontains='经济') | Q(category__in=economic_categories)
    )
