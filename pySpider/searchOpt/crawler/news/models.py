...
class FeaturedSelection(models.Model):
    ...
    selection_mode = models.CharField(
        max_length=32, default="tech4-econ2-mil2", verbose_name="精选模式"
    )
    ...
    image_url = models.CharField(
        max_length=500, default="", blank=True, verbose_name="图片链接"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["selection_date", "slot_order"],
                name="uniq_featured_selection_slot",
            ),
        ]
        indexes = [
...
class HotProduct(models.Model):
    ...
    image_url = models.CharField(
        max_length=500, verbose_name="图片链接", blank=True, default=""
    )
...
class AiSearchTask(models.Model):
    ...
    target_url = models.CharField(
        max_length=500, blank=True, default="", verbose_name="目标URL"
    )
    category = models.CharField(
        max_length=20, choices=CATEGORY_CHOICES, default="all", verbose_name="分类"
    )
    ...
    error_msg = models.CharField(
        max_length=500, blank=True, default="", verbose_name="错误信息"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
...
class OriginalArticle(models.Model):
    ...
    url = models.CharField(
        max_length=500, verbose_name="原文链接", blank=True, help_text="微信文章或外部链接"
    )
    image_url = models.CharField(
        max_length=500, verbose_name="封面图片", blank=True, default=""
    )
    source_type = models.CharField(
        max_length=20, choices=SOURCE_CHOICES, default="original", verbose_name="来源类型"
    )
