...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--days", type=int, default=2, help="分析最近N天的数据（默认2天）")
        parser.add_argument("--top", type=int, default=5, help="保存前N个热门产品（默认5个）")
    ...
    def handle(self, *args, **options):
        ...
        HotProduct.objects.filter(
            period_type="weekly", period_start=start_date
        ).delete()
