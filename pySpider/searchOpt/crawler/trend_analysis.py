...
class TrendAnalyzer:
    ...
    def analyze(self):
        monthly_data = defaultdict(
            lambda: {
                cat: {"score": 0.0, "keywords": Counter()} for cat in self.CATEGORIES
            }
        )
        try:
            ...
            for title, rate, db_cat, publish_time in rows:
                if not title or not publish_time or len(publish_time) < 10:
                    continue
                    ...
                    series_data.append(
                        {
                            ...
                            "meta": {
                                ...
                                "events": top_events_str if top_events_str else "无显著事件",
                            },
                            "month": t_key,
                        }
                    )
            ...
            json_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "trend_data.json"
            )
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(final_json, f, ensure_ascii=False, indent=2)

            print(
                f"Analysis complete. RowCount={len(rows)} MatchedCount={count_matched} Points={len(sorted_times)}"
            )
            return True
