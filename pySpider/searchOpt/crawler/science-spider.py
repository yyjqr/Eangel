...
try:
    ...
    # 备用：如果导入失败，定义一个简单的
    def extract_date_from_url_or_title(url, title):
        return None
        ...
        for title, url, weight, image_url, pub_time in self.articles:
            if weight > kRankLevelValue:
                ...
                newsOne = (
                    ...
                    self.source_name,
                    publish_time_str,
                    "Science and Mathematics discovery",
                    url,
                    "Science,Math",
                    final_category,
                    image_url,
        ...
        try:
            ...
            for a in links:
                ...
                url_date_match = re.search(
                    r"/releases/(\d{4})/(\d{2})/(\d{2})(\d{2})(\d{2})", url
                )
                if url_date_match:
                    try:
                    ...
                    if extracted and len(extracted) >= 10:
                        try:
                            pub_time = datetime.strptime(extracted[:10], "%Y-%m-%d")
                        except:
                            pass

                weight = self.calculate_weight(title)
                # 排除已经处理过的（基础去重）
                if any(art[1] == url for art in self.articles):
                    continue
        ...
        try:
            ...
            for entry in feed.entries[:10]:
                ...
                if hasattr(entry, "published_parsed") and entry.published_parsed:
                    ...
                    if extracted and len(extracted) >= 10:
                        try:
                            dt = datetime.strptime(extracted[:10], "%Y-%m-%d")
                        except:
                            pass
