...
class MilitaryScraperBase:
    ...
    def fetch_article_summary(self, url: str, max_sentences: int = 3) -> str:
        ...
        try:
            ...
            article_body = (
                ...
                or soup.find(
                    "div",
                    class_=re.compile(r"article|content|post|entry|story|body", re.I),
                )
                or soup.find(
                    "div", id=re.compile(r"article|content|post|entry|story|body", re.I)
                )
                or soup
            )
    ...
    def scrape_links_from_page(
        ...
        for node in candidates:
            ...
            anchor = (
                node if getattr(node, "name", "") == "a" else node.find("a", href=True)
            )
            if not anchor:
                continue
            ...
            if url.startswith("javascript:") or url.startswith("mailto:"):
                continue
            if (
                parsed_page.netloc not in url_host
                and "ifeng.com" not in url_host
                and "huanqiu.com" not in url_host
            ):
                continue
            if any(pattern in url_lower for pattern in exclude_patterns):
                continue
            if allow_patterns and not any(
                pattern in url_lower for pattern in allow_patterns
            ):
                continue
            ...
            articles.append(
                self.build_article(
                    title, url, summary=summary, tags=tags, image_url=image_url
                )
            )
    ...
    def scrape_articles(self, limit: int = 10) -> List[Dict]:
        try:
            articles = self.scrape_links_from_page(
                self.base_url,
                selectors=[
                    'a[href*="huanqiu.com/article/"]',
                    'a[href*="mil.huanqiu.com/article/"]',
                    "h2 a",
                    "h3 a",
                ],
                limit=limit,
