...
class NewsScraper:
    ...
    def filter_and_store(self, keywords="军事"):
        ...
        for article_tuple in self.articles:
            ...
            try:
                db_publish_time = parse_db_publish_time(
                    mysqlWriteNewsV2.getArticlePublishTime(url)
                )
                if db_publish_time:
    ...
    def scrape(self, limit=10):
        ...
        for art in articles:
            weight = self.calculate_weight(
                art.get("title", ""), art.get("summary", ""), art.get("tags", "")
            )
...
class MilitaryNewsAggregator:
    ...
    def build_email_html(self, articles):
        ...
        for idx, article in enumerate(articles, 1):
            ...
            show_summary = summary and summary.strip().rstrip(
                "。."
            ) != title.strip().rstrip("。.")

            html_parts.append(
                f'<p style="margin-bottom:4px;"><b>{idx}.</b> '
                f'[{weight:.2f}] <a href="{url}" style="color:#1a73e8;text-decoration:none;">{title}</a></p>'
            )
            if isinstance(publish_time, datetime):
                html_parts.append(
                    f'<p style="margin:0 0 2px 24px;font-size:12px;color:#888;">📅 {publish_time.strftime("%Y-%m-%d %H:%M")}</p>'
                )
            if show_summary:
                html_parts.append(
                    f'<p style="margin:0 0 12px 24px;font-size:13px;color:#555;">{summary[:300]}</p>'
                )
            else:
