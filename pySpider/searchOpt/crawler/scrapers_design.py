            ...
            for item in items:
                try:
                    ...
                    title_elem = item.select_one(
                        "h2, h3, h4, .headline, .teaser__headline"
                    )
                    if not title_elem:
                    ...
                    # 提取图片
                    image_url = (
                        self.extract_image_url(item, ["img", "[data-src]"]) or ""
                    )
        ...
        try:
            ...
            for item in items:
                try:
                    ...
                    if not title:
                        title = "leManoosh Design Inspiration"

                    # 提取图片 (leManoosh 图片很重要)
                    image_url = (
                        self.extract_image_url(item, ["img", ".post-thumbnail img"])
                        or ""
                    )
...
class BehanceScraper(DesignScraper):
    ...
    def __init__(self):
        ...
        # 使用工业设计分类的搜索页面
        self.base_url = (
            "https://www.behance.net/galleries/industrial-design/product-design"
        )
        self.source = "Behance"
    ...
    def scrape_articles(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            # 尝试抓取项目卡片
            items = soup.select(
                ".ProjectCover-root-s3g, .ContentGrid-gridItem-W_3, li.rf-project-cover"
            )
                ...
                for script in scripts:
                    if script.string and '"projects":' in script.string:
                        try:
                            ...
                            # 仅作演示，实际可能不稳定
                            matches = re.findall(
                                r'"url":"(https://www.behance.net/gallery/[^"]+)","title":"([^"]+)"',
                                script.string,
                            )
            ...
            for item in items[:limit]:
                try:
                    title_elem = item.select_one(
                        ".ProjectCover-details-title, .Title-title-3nk, h3"
                    )
                    ...
                    image_url = (
                        self.extract_image_url(
                            item, ["img.ProjectCover-image-3mz", "img"]
                        )
                        or ""
                    )
            ...
            for entry in feed.entries[:limit]:
                ...
                if is_target or True:
                    ...
                    summary = self.clean_text(
                        re.sub(r"<[^>]+>", "", entry.get("summary", "")[:300])
                    )
