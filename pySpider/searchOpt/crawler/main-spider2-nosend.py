...
def extract_date_from_url_or_title(
    url: str, title: str, session: requests.Session = None, fetch_details: bool = False
) -> Optional[str]:
        ...
        for news_element in news_elements:
            try:
                ...
                if title_elem and link_elem:
                    ...
                    pubdate_str = extract_date_from_url_or_title(
                        url,
                        title,
                        session=self.session,
                        fetch_details=FETCH_DETAILS_FOR_DATE,
                    )
                    ...
                    if weight > 0:
                        ...
                        self.articles.append(
                            (title, url, weight, image_url, pubdate, datetime.now())
                        )
            except Exception as e:
