/**
基于opencv4.1.0，c++
类和函数的模式，编写读取摄像头拍摄视频视频帧，保存为以时间命名的文件。另同时创建了录音线程。
...
int main(int argc, char **argv) {
  ...
  if (videoCapturer.isOpened()) {
    ...
    // //这个参数获取会报错！！ VIDEOIO ERROR: V4L2: getting property #7 is not
    // supported
    ...
    cout << " " STR_OK " video info"
         ...
         << ", FPS: " << videoCapturer.get(CAP_PROP_FPS) << ", FrameCount: "
         << " "
  ...
  t = time(
      &timep); // 放在循环里面才行，外面的话，时间是一个固定的，不符合要求！！！0907
  ...
  // VideoWriter writer(pVideoFileName, CV_FOURCC('M', 'P','4', '2'),
  // videoCapturer.get(CAP_PROP_FPS),Size(videoCapturer.get(CAP_PROP_FRAME_WIDTH),videoCapturer.get(CAP_PROP_FRAME_HEIGHT)));//AVI
  // 0901   avi格式 MJPG编码
  VideoWriter writer(pVideoFileName, VideoWriter::fourcc('M', 'P', '4', '2'),
...
void *record_thread(void *args) {
  ...
  */
  ...
  sprintf(play_cmd, "arecord  -f cd -t wav -r 10000 -D plughw:1,0 %s%s.wav",
          str_saveDir.c_str(), buf); // buf 为时间名称
  if (recordFlag) {
