...
inline bool is_perfect_square(int128 n) {
  // 快速位运算模256、模255剪枝
  if (!valid_sq256[n & 255])
    return false;
  if (!valid_sq255[n % 255])
    return false;
  if (!valid_sq63[n % 63])
    return false;

  long long root = sqrt((double)n);
  if ((int128)root * root == n)
    return true;
  if ((int128)(root + 1) * (root + 1) == n)
    return true;
  if ((int128)(root - 1) * (root - 1) == n)
    return true;
  return false;
...
void find_cuboid_worker(long long a_start, long long a_end, long long max_len) {
  for (long long a = a_start; a < a_end; ++a) {
    ...
    for (long long b = a + 1; b <= max_len; ++b) {
      // (1) 如果 a 是奇数且 b 也是奇数，a^2+b^2 会是 2(mod 4)，绝不可能是平方数
      // -> b 必为偶数
      if (a_mod4 % 2 != 0 && b % 2 != 0)
        continue;
      ...
      for (long long c = b + 1; c <= max_len; ++c) {
        // 性质剪枝集成
        if (a_mod3 != 0 && b_mod3 != 0 && c % 3 != 0)
          continue;
        if (a_mod5 != 0 && b_mod5 != 0 && c % 5 != 0)
          continue;
        ...
        // 2. 检查另外两个面对角线
        if (!is_perfect_square(a2 + c2))
          continue;
        if (!is_perfect_square(b2 + c2))
          continue;
...
int main() {
  ...
  for (int i = 0; i < num_threads; ++i) {
    ...
    workers.emplace_back(find_cuboid_worker, start, end,
                         1500000000000LL); // max_c 为 1.5E15
  }
