  ...
  for (long long a = start_number; a <= max_length; a++) {
    ...
    for (long long b = a + 1; b <= max_length; b++) {
      ...
      if ((a + c) < b)
        continue;
      if (a == b || a == c || b == c)
        continue; // add 两边相等，不符合 08.20
      // b+c>a+1>a
      // cout <<"threadId:" <<threadId <<"searchTimes:"<<searchTimes<<endl;
      if (searchTimes % 10000 == 0) {
        cout << "threadId:" << threadId << "searchTimes:" << searchTimes
             << endl;
        if (a > 20 && b > 20 && c > 20) {

          cout << "searchTimes:" << searchTimes << "step,try a cube"
               << "a|b|c:" << a << "," << b << "," << c << endl;
        }
      }
