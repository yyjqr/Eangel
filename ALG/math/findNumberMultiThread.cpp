  ...
  for (unsigned long long int i = beginNumber; i < endNumber; i += 2) {
    // if(i%2!=0 )
    {
      if (i < 32) {
        ...
        if (a % i == 0) {
          count++;
          cout << ">>>>find a special number:" << i << "***index" << count
               << endl
               << endl;
        }
        ...
        if (i % 11 != 0 && i % 17 != 0 && i % 13 != 0 && i % 19 != 0 &&
          ...
          if (number_sqrt_value - number_sqrt < kValue_gap) {
            ...
            // cout<<i<<",Not a special number ,sqrt
            // value:"<<number_sqrt_value<<endl;
            continue;
          }
          if (i > 1000) {
          ...
          if (!b_not_a_special_number) {
            ...
            count++;
            mtx.unlock();
            cout << "maybe a special  number:" << i << "***index" << count
                 << endl;
...
int main() {
  ...
  thread th[10];
  for (int i = 0; i < 10; i++) {
    //    th[i] = thread(findSpecialNumber,
    //    1+oneStepBaseNumber*i,oneStepBaseNumber*(i+1),numberStream);
    th[i] = thread(findSpecialNumber, i, 1 + oneStepBaseNumber * i,
                   oneStepBaseNumber * (i + 1));
  }
  for (int i = 0; i < 10; i++) {
    th[i].join();
  }
