...
public:
  HelloWorldPublisher()
      : participant_(nullptr), publisher_(nullptr), topic_(nullptr),
        writer_(nullptr), type_(new RobotSensorDataPubSubType()) {}
  ...
  bool init() {
    ...
    // TOPIC_QOS_DEFAULT);
    topic_ = participant_->create_topic("Road1-RSM", "HelloWorld",
                                        TOPIC_QOS_DEFAULT);
    if (topic_ == nullptr) {
      return false;
    }
  ...
  void run(uint32_t samples) {
    ...
    while (samples_sent < samples) {
      if (publish()) {
        samples_sent++;
        timestamp64 = std::chrono::duration_cast<std::chrono::milliseconds>(
                          std::chrono::steady_clock::now().time_since_epoch())
                          .count();
        // std::cout << "ts:"<<timestamp64<<"Message: " << robotInfo_.message()
        // << " with index: " << robotInfo_.index()
