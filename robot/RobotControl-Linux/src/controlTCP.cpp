...
controlTCP::controlTCP(QObject *parent) : QTcpSocket(parent) {
...
bool controlTCP::disconnectSocket() {
  qDebug() << "pictureSocket state  :" << pictureSocket->state();
  if (pictureSocket->state() == QTcpSocket::ClosingState ||
      pictureSocket->state() == QTcpSocket::UnconnectedState) {
    return true;
  } else {
...
void controlTCP::recvData(void) {
  ...
  mutex.lock();
  while (pictureSocket->waitForReadyRead(
      300)) // 200--->300 尽量读取到1张图像的数据 20211023
  {
    //        bytes.append((QByteArray)pictureSocket->readAll());
    bytes.append(
        (QByteArray)pictureSocket->read(CAM_ResolutionRatio * IMAGESIZE));
    if (bytes.size() >= CAM_ResolutionRatio * IMAGESIZE &&
        bytes.size() < CAM_ResolutionRatio * IMAGESIZE * 1.5) {
...
QByteArray controlTCP::getOneFrameDATA() {
  ...
  if (m_queue_camDataInCHAR.size() != 0) {
    qDebug() << " ------Get m_queue_camDataInCHAR.size():"
             << m_queue_camDataInCHAR.size() << "\n";
    m_byteArray_oneFrame = m_queue_camDataInCHAR.front();
    // 断开后，再次连接可能出错的地方 202201
    m_queue_camDataInCHAR.pop_back();
    //        qDebug()<<" After get,
    //        m_queue_camDataInCHAR.size():"<<m_queue_camDataInCHAR.size()<<
    //        "\n";
