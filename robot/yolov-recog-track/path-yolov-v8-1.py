...
class YOLOv8Node(Node):
    def __init__(self):
        ...
        try:
            ...
            # Outgoing stream to PC (H264 over UDP).
            # If enable_out_stream is True, the USB stream is split (Tee) -> one branch to UDP, one to Appsink
            self.declare_parameter("enable_out_stream", True)
            self.declare_parameter("stream_host", "192.168.3.62")
            ...
            enable_out = (
                self.get_parameter("enable_out_stream").get_parameter_value().bool_value
            )
            stream_host = (
                self.get_parameter("stream_host").get_parameter_value().string_value
            )
            stream_port = (
            ...
            if enable_out:
                if not GI_AVAILABLE:
                    self.get_logger().error(
                        "GStreamer python bindings not found, but enable_out_stream is True. Falling back to simple USB."
                    )
                    enable_out = False
                else:
                    # Use GStreamer pipelie to split stream (Tee) -> [Stream, Appsink]
                    # Note: appsink must be named 'mysink' for GStreamerCapture
                    self.get_logger().info(
                        f"Opening USB camera {cam_dev} with Tee to {stream_host}:{stream_port}"
                    )
                    # Updated pipeline: 20fps, name=mysink, explicit format for x264
                    gst_str = (
                        f"v4l2src device={cam_dev} ! "
                        f"video/x-raw, width={cam_w}, height={cam_h}, framerate=20/1 ! "
                        "tee name=t "
                        f"t. ! queue leaky=1 ! videoconvert ! video/x-raw,format=I420 ! x264enc tune=zerolatency speed-preset=ultrafast bitrate=2000 ! "
                        f"rtph264pay config-interval=1 ! udpsink host={stream_host} port={stream_port} sync=false "
            ...
            if not enable_out:  # If check above failed or disabled
                # Normal USB capture
