...
#define TEXT_COLOR_RED(STRING)                                                 \
  "<font color=red>" STRING "</font>"                                          \
  "<font color=black> </font>"
#define TEXT_COLOR_BLUE(STRING)                                                \
  "<font color=blue>" STRING "</font>"                                         \
  "<font color=black> </font>"
#define TEXT_COLOR_GREEN(STRING)                                               \
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow), b_grabPic(false),
      m_saveIndex(0), m_getImageCount(0), showThread(nullptr),
  ...
  // 增加失去服务器连接的相关操作
  connect(showThread, SIGNAL(SIGNAL_camSocketDisconnectToMainThread()), this,
          SLOT(disconnect_Deal()));
  //    connect(showThread, SIGNAL(finished()), showThread,
  //    SLOT(deleteLater()));//信号槽中已有删除，析构函数中使用的回收资源更清晰！！！
}
...
void MainWindow::on_pushButtonConnect_clicked() {
  ...
  if (showThread->connectTCPSocket(addr)) {
    qDebug() << "\n Test fun:" << __func__ << "showThread ptr:" << showThread
             << endl;
  ...
  if (m_picToshow.imageBuf != nullptr) {
    ...
    if (m_getImageCount % 2 == 0) {
      qDebug()
          << "\n"
          << __LINE__
          << "frameToShow -----"; // <<"frameToShow.imageBuf:"<<m_picToshow.imageBuf;
...
void MainWindow::onTimeGetFrameToShow() {
  ...
  // 未获取到图像数据，会一直在空指针下，再获取一帧，会分配内存，导致内存过大！
  // 1023
  if (m_picToshow.imageBuf != nullptr) {

    //        qDebug() << "Get the
    //        data,m_picToshow.imageBuf:"<<m_picToshow.imageBuf;
    getPicToShow();
    qDebug() << "After show:" << __func__
             << "picToshow.imageBuf:" << m_picToshow.imageBuf;
    m_picToshow.imageBuf = nullptr; // 显示后，将其置空 0717--->
    ui->label_RecvPictureNums->setText(QString::number(m_getImageCount));
  } else {
    //      qDebug() << "fun: "
    //      <<__func__<<"picToshow.imageBuf:"<<m_picToshow.imageBuf;
...
bool MainWindow::ShowImage(uint8_t *pRgbFrameBuf, int nWidth, int nHeight,
  ...
  image = QImage(pRgbFrameBuf, nWidth, nHeight,
                 QImage::Format_RGB888); // Format_BGR888 --->Format_RGB888
  if (b_grabPic == true) {
    QDateTime datetime;
    ...
    ui->textBrowser_log->append(
        timestrLog +
        QString::asprintf("截图成功%s", SAVE_NAME.toStdString().c_str()));
    m_saveIndex++;
    b_grabPic = false;
  }
...
void MainWindow::on_pushButton_disconnect_clicked() {
  ...
  QString tmp_qstr =
      "<font color=\"#FF0000\">" + timestr + "服务器断开连接" + "</font>";
