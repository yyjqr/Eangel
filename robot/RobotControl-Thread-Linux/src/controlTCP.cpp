...
controlTCP::controlTCP(QObject *parent)
    : QTcpSocket(parent), m_NoDataTimes(0), b_realStopTimer(false) {
...
bool controlTCP::disconnectSocket() {
  qDebug() << "m_camSocket state  :" << m_camSocket->state();
  if (m_camSocket->state() == QTcpSocket::ClosingState ||
      m_camSocket->state() == QTcpSocket::UnconnectedState) {
    return true;
  } else {
...
void controlTCP::recvData(void) {
  ...
  mutex.lock();
  while (m_camSocket->waitForReadyRead(
      500)) // 200--->300 尽量读取到1张图像的数据 20211023
  {
    //        bytes.append((QByteArray)m_camSocket->readAll());
    bytes.append(
        (QByteArray)m_camSocket->read(CAM_ResolutionRatio * IMAGESIZE));
