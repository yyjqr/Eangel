...
CamThread::CamThread()
    : b_run(true), oneCamInfo({nullptr, 1280, 720, 0}),
      oneFrameInfo({nullptr, 1280, 720, 0}), b_dataValid(false),
      m_countNOdata(0), m_pictureSocket(nullptr), imageExtraDataBuf(nullptr),
      m_tryGetDataTimes(0), m_countGet(0), m_malloc_times(0) {
...
void CamThread::run() {
  ...
  connect(m_pictureSocket, SIGNAL(signalSocketDisconnect()), this,
  ...
  while (b_run) {
    ...
    if (m_getFrame_byteArray.length() > 0) {
      //            qDebug()<<"m_getFrame_byteArray length:
      //            "<<m_getFrame_byteArray.length();
      // 比正常数据多1.5倍,可尝试拷贝,如果多出10倍,拷贝报错  0927
      // m_getFrame_byteArray length:  1987737462
      if (m_getFrame_byteArray.length() <=
          IMAGESIZE * CAM_ResolutionRatio * 1.5) {
        //                  qDebug()<<"analysis mem\n";
...
void CamThread::receiveValidPicture(QByteArray bytes) {
  if (bytes.size() >= IMAGESIZE * CAM_ResolutionRatio) {
    oneCamInfo.imageBuf = (uint8_t *)malloc(
        sizeof(uint8_t) * IMAGESIZE * CAM_ResolutionRatio); // 分配内存 RGB 3倍
    //       qDebug() <<"malloc mem times:"<<m_malloc_times++<<endl;
    if (oneCamInfo.imageBuf != nullptr) {
      //            memcpy(oneCamInfo.imageBuf,bytes,bytes.size()>IMAGESIZE*CAM_ResolutionRatio
      //            ? IMAGESIZE*CAM_ResolutionRatio:bytes.size());
    ...
    qDebug() << "cam size small and clear, bytes.size():" << bytes.size()
             << "\n";
    //        bytes.clear();  //字节少的时候,是否需要释放?? cam size small and
    //        clear, bytes.size(): 2092360
  }
}
...
void CamThread::receivePic(QByteArray bytes) {
  ...
  if (bytes.size() > 0) {
      ...
      if (extraDataSize > 0) {
        ...
        if (extraDataSize > 0 && extraDataSize < IMAGESIZE) {
          ...
          if (IMAGESIZE * 3 - extraDataSize < bytes.size()) {
            qDebug() << __func__ << __LINE__ << "IMAGESIZE*3-extraDataSize="
    ...
    qDebug() << " \n -----Total bytes.size() " << bytes.size()
             << "------------ \n";
    // 只把读取是完整字节及以上的数据存到队列
    // 0620----->bytes.size()+extraDataSize  0626
    ...
    if (bytes.size() + extraDataSize >= IMAGESIZE * 3) {
      ...
      qDebug() << __LINE__
               << "read bytes.size()+extraSize<IMAGESIZE*3,free mem...\n";
...
bool CamThread::getOneFrame() {
  ...
  if (camSaveQueue.size() != 0) {
    ...
    qDebug() << "num:" << m_countGet++
             << " getOneFrame After get, camSaveQueue.size() "
             << camSaveQueue.size() << endl;
...
camInfo CamThread::getCamOneFrame() {
  QMutexLocker locker(
      &m_dataMutex); // 主线程取数据,也得加锁,避免同时被取到!!!0929
  if (camSaveQueue.size() != 0) {

    oneFrameInfo = camSaveQueue.front();
    camSaveQueue.pop();
    //          qDebug() << "\n fun: " <<__func__<<__LINE__<<"get one frame to
    //          show\n ";
    //        qDebug() << "Return
    //        oneFrameInfo.imageBuf:"<<oneFrameInfo.imageBuf;
...
void CamThread::setThreadFlag(bool b_runFlag) { b_run = b_runFlag; }
