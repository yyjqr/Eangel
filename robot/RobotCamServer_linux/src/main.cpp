...
int main(int argc, char **argv) {
  ...
  Absolute_exec_path =
      file_get_exec_absolute_path.get_cur_executeProgram_path();
  string timestr;
...
void camReadFunc() {
  ...
  video_capture.open(
      -1); // 从摄像头读入视频 0表示从摄像头读入  -1表示任意摄像头 202003
    ...
    for (;;) {
      ...
      if (frame.isContinuous()) {
        ...
        if (st_oneFrame.camPtr != nullptr) {
          memcpy(st_oneFrame.camPtr, rgbFrame.data,
                 rgbFrame.rows * rgbFrame.cols * channel);
...
void threadFunc() {
  ...
  while (1) {
    ...
    //    boost::posix_time::time_duration now_time_of_day =
    //    boost::posix::microsec_clock::local_time().; cout<<"start
    //    time:"<<pTime<<endl; std::string strTimeOfDay =
    //    boost::posix_time::to_simple_string(startTime.time_of_day()); //
    //    当前时间：15:03:55 cout<<"day time:"<<strTimeOfDay<<endl;

    memset(recvCMD, '\0', sizeof(recvCMD));
    b_recvStatus = camSocket.recvData(recvCMD, sizeof(recvCMD));
    boost::posix_time::ptime secondTime =
        boost::posix_time::microsec_clock::local_time();
    strTimeOfDay = boost::posix_time::to_simple_string(
        secondTime.time_of_day()); // 当前时间：15:03:55
    ...
    if (b_recvStatus) {
      ...
      if (strcasecmp(recvCMD, "PIC") == 0) {
        ...
        if (cam_deque.size() > 0) {
          ...
          cout << "After send and free,cam deque size:" << cam_deque.size()
               << endl;
        }
      }
    } else {
