...
bool tcpSocket::connectSocket() {
  ...
  //    //这里可以改成服务端的IP，若是都在一台电脑就不用改变
  serAddr.sin_addr.s_addr =
      htonl(INADDR_ANY); // 这里可以改成服务端的IP，若是都在一台电脑就不用改变
  ...
  m_connfd = accept(m_server_sockfd, (struct sockaddr *)&client_address,
                    &client_addrlength);
  ...
  if (m_connfd < 0) {
    ...
    getpeername(m_connfd, (struct sockaddr *)&client_address,
                &client_addrlength);
