...
enum AVFrameSideDataType {
   ...
   * AV_FRAME_QP_TABLE_DATA. The contents of this side data are undocumented and
   * internal; use av_frame_set_qp_table() and av_frame_get_qp_table() to access
   * this in a meaningful way instead.
   */
...
#if FF_API_FRAME_QP
attribute_deprecated int8_t *av_frame_get_qp_table(AVFrame *f, int *stride,
                                                   int *type);
attribute_deprecated int av_frame_set_qp_table(AVFrame *f, AVBufferRef *buf,
                                               int stride, int type);
#endif
attribute_deprecated enum AVColorSpace
...
attribute_deprecated enum AVColorRange
