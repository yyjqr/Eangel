...
typedef struct AVVulkanDeviceContext {
   ...
   * VK_KHR_external_semaphore_fd and VK_EXT_external_memory_host are enabled if
   * found. If supplying your own device context, these fields takes the same
   * format as the above fields, with the same conditions that duplicates are
   * possible and accepted, and that NULL and 0 respectively means no extensions
   * are enabled.
   */
   ...
   * enabled during device creation. When a device is created by FFmpeg, it will
