...
void MainWindow::on_startButton_clicked() {
  ...
  if (flag) {
    ...
    connect(mPlayer, SIGNAL(sig_GetOneFrame(QImage)), this,
            SLOT(slotGetOneFrame(QImage)));
    connect(mPlayer, SIGNAL(sig_StreamError(QString)), this,
            SLOT(handleStreamError(QString)));
    ...
    qDebug() << "fun: " << __func__ << "line:" << __LINE__
             << "analysis stop play or rtsp server stop (connect disconnect)";
    disconnect(mPlayer, SIGNAL(sig_GetOneFrame(QImage)), this,
               SLOT(slotGetOneFrame(QImage)));
    mPlayer->stopPlay();
    qDebug() << __func__ << "line:" << __LINE__
             << "play or connect error issue";
    mPlayer_run_flag = false;
...
void MainWindow::saveImage(QImage img) {
  ...
  //        assert(pRgbFrameBuf!=nullptr); //增加，来测试是否截图时，出现错误
  //        1128 qDebug()<<__LINE__<<" inside,test grab error";
...
void MainWindow::on_pushButton_CARconnect_clicked() {
  qDebug() << "first test connect:" << m_str_addr
           << "controlSocket->state():" << p_controlSocket->state();
  p_controlSocket->connectToHost(m_str_addr, 6868); // 车的控制端口，6868
  qDebug() << "test connect:" << m_str_addr
           << "p_controlSocket->state():" << p_controlSocket->state();
  LogInfo("connect robot ip:%s, state:%d", m_str_addr.toStdString().c_str(),
          p_controlSocket->state());
  if (p_controlSocket->state() == QTcpSocket::ConnectingState) {
    ui->textBrowser_log->append(m_timestr + "正在连接:" + m_str_addr +
                                " Robot---");
  }
  p_controlSocket->waitForConnected(3000);
  if (p_controlSocket->state() == QTcpSocket::ConnectedState) {
    ui->textBrowser_log->append(m_timestr + "机器人连接:" + m_str_addr +
                                " OK++");
    LogInfo("connect robot OK,ip:%s, state:%d",
            m_str_addr.toStdString().c_str(), p_controlSocket->state());
    //        initControlUI(true);
  } else {
    ui->textBrowser_log->append(m_timestr + "机器人连接失败--");
    LogWarning("connect robot ip:%s, state:%d",
               m_str_addr.toStdString().c_str(), p_controlSocket->state());
...
void MainWindow::writeCmdToSocketBuf(char cmd) {
  ...
  if (ret < 0) {
    ui->textBrowser_log->append(m_timestr + "写入命令:" + cmd + " " + "failed" +
...
void MainWindow::writeCmdStringToSocketBuf(char *pCmd) {
  long ret = -1;
  // 对写入命令是否成功做判断 2023.06
  qDebug() << __LINE__ << "test send CAR CMD:" << *pCmd << "strlen(pCmd)"
           << strlen(pCmd);
  ret = p_controlSocket->write(pCmd, strlen(pCmd) + 1);
  LogInfo("send long cmd to robot,cmd:%s, ret:%d", pCmd, ret);
  if (ret < 0) {
    ui->textBrowser_log->append(m_timestr + "写入命令:" + pCmd + " " +
                                "failed" + " ret:" + QString::number(ret));
  }
}

void MainWindow::STOP() {
  char buf =
      'P'; // UNO单片机端，目前只支持单字母。 （python端，支持字符串） 2023.05
...
void MainWindow::on_pushButtonCARBACK_clicked() { goBack(); }

void MainWindow::on_pushButton_turnLeft_clicked() { goLeft(); }

void MainWindow::on_pushButton_turnRight_clicked() { goRight(); }
...
void MainWindow::ParseFromJson() {
  ...
  if (!ifs) {
    ...
    for (int i = 0; i < json_flow.size(); i++) {
      ...
      qDebug() << "test one_dev_json,line:" << __LINE__
      ...
      qDebug() << "test one_dev_json,line:" << QString::fromStdString(str_ip1)
               << " " << QString::fromStdString(str_rtsp_url);
...
void MainWindow::on_pushButton_disconnect_clicked() {
  QString tmp_qstr =
      "<font color=\"#FF0000\">" + m_timestr + "断开连接" + "</font>";
  if (p_controlSocket->state() == QTcpSocket::ConnectedState) {
...
void MainWindow::onDisconnect() {
  QString tmp_qstr =
      "<font color=\"#FF0000\">" + m_timestr + "服务器断开连接" + "</font>";
  ui->pushButton_CARconnect->setEnabled(true);
}
...
void MainWindow::on_pushButton_clearCommand_clicked() { cleanCommandBuffer(); }
