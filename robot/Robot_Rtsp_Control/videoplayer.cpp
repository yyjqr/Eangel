...
VideoPlayer::~VideoPlayer() { qDebug() << "播放器已经释放"; }
...
void VideoPlayer::stopPlay() {
  qDebug() << getCurrentTime() << "stopFlag:" << stopFlag
           << "QThread::isRunning():" << QThread::isRunning();
...
void VideoPlayer::setFileName(QString name) { this->mFileName = name; }

QString VideoPlayer::getFileName() { return mFileName; }
...
void VideoPlayer::run() {
    ...
    if (packet->stream_index == videoStream) {
      ...
      if (got_picture) {
        ...
        if (m_not_get_times > 3) {
          ...
          QMessageBox *box =
              new QMessageBox(QMessageBox::Warning, tr("告警"), "取流失败");
  ...
  QMessageBox *box =
      new QMessageBox(QMessageBox::Warning, tr("告警"), "取流失败,结束取流");
  QTimer::singleShot(4500, box, SLOT(accept())); // 1.5s弹框自动消失
  box->exec();
  delete box;
  qDebug() << getCurrentTime()
           << "rtsp  thread exit,ID: " << QThread::currentThreadId();
}
