...
MyThread::MyThread()
    : b_run(true), oneCamInfo({nullptr, 1280, 720, 0}),
      ...
      m_countNOdata(0), imageExtraDataBuf(nullptr), m_tryGetDataTimes(0) {
...
void MyThread::run() {
  ...
  int i = 0;
  connect(m_pictureSocket, SIGNAL(dataReady(QByteArray)), this,
          SLOT(receiveValidPicture(QByteArray)));
...
bool MyThread::getOneFrame() {
  ...
  if (camSaveQueue.size() != 0) {
    ...
    qDebug() << "getOneFrame After get, camSaveQueue.size() "
             << camSaveQueue.size();
...
void MyThread::setThreadFlag(bool b_runFlag) { b_run = b_runFlag; }
