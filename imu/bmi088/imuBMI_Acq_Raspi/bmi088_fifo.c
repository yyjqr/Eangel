 ...
 * @return None
 */
static void check_empty_accel_fifo(uint16_t *data_index,
                                   const struct bmi088_dev *dev);
 ...
 * @return None
 */
static void extract_accel_header_mode(struct bmi088_sensor_data *accel_data,
 ...
 * @return None
 */
static void unpack_accel_frm(struct bmi088_sensor_data *accel, uint16_t *idx,
                             uint16_t *accel_idx, uint8_t frm,
                             const struct bmi088_dev *dev);
 ...
 *
 * @param[out] accel_data        : Structure instance of bmi088_sensor_data
 * where the parsed accel data bytes are stored.
 ...
 *
 */
static void unpack_accel_data(struct bmi088_sensor_data *accel_data,
 ...
 * @return None
 */
static void unpack_accel_skipped_frame(uint16_t *data_index,
                                       const struct bmi088_dev *dev);
 ...
 * @return None
 */
static void unpack_accel_dropped_frame(uint16_t *data_index,
                                       const struct bmi088_dev *dev);
 ...
 * @return None
 */
static void move_to_next_accel_frame(uint16_t *data_index,
 ...
 * @return None
 */
static void get_gyro_len_to_parse(uint16_t *start_idx, uint16_t *len,
                                  const uint16_t *gyro_count,
                                  const struct bmi088_dev *dev);
 ...
 *                            to be parsed
 *  @param[in] frm          : It consists of either the fifo_data_enable
 * parameter (Gyro data enabled in FIFO) in header-less mode
 *  @param[in] dev          : Structure instance of bmi088_dev.
 *
 *  @return Result of API execution status
 *  @retval 0 -> Success
 ...
 *
 */
static uint16_t unpack_gyro_frm(struct bmi088_sensor_data *data, uint16_t *idx,
                                uint16_t *gyro_idx, uint8_t frm,
                                const struct bmi088_dev *dev);
 ...
 * gyro_data.
 *
 * @param[out] gyro_data        : Structure instance of bmi088_sensor_data where
...
static void unpack_gyro_data(struct bmi088_sensor_data *gyro_data,
                             uint16_t start_idx, const struct bmi088_dev *dev);
 ...
 * @return None
 */
static void check_empty_gyro_fifo(uint16_t *data_index,
                                  const struct bmi088_dev *dev);
...
uint16_t bmi088_get_accel_fifo_data(struct bmi088_dev *dev) {
  ...
  if (rslt == BMI088_OK) {
    ...
    rslt |= dev->read(dev->accel_id, reg_addr, dev->accel_fifo->data,
                      dev->accel_fifo->length);
 ...
 */
uint16_t bmi088_extract_accel(struct bmi088_sensor_data *accel_data,
                              uint16_t *accel_length,
                              const struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  ...
  if (rslt == BMI088_OK) {
    ...
    if (dev->accel_fifo->fifo_header_enable == 0) {
      ...
      for (; data_index < data_read_length;) {
        ...
        unpack_accel_frm(accel_data, &data_index, &accel_index,
                         dev->accel_fifo->fifo_data_enable, dev);
...
uint16_t bmi088_set_accel_fifo_wm(uint16_t fifo_wm, struct bmi088_dev *dev) {
  ...
  if (rslt == BMI088_OK) {
    ...
    /* Write the Accel fifo watermark level */
    rslt |= bmi088_set_accel_regs((reg_addr + BMI088_ONE), &data[1], BMI088_ONE,
                                  dev);
  }
 ...
 *  or unfiltered mode.
 */
uint16_t bmi088_get_accel_fifo_filt_data(uint8_t *accel_fifo_filter,
  ...
  if (rslt == BMI088_OK) {
    ...
    if (rslt == BMI088_OK) {
      ...
      *accel_fifo_filter =
          BMI088_GET_BITSLICE(data, BMI088_ACCEL_FIFO_FILT_DATA);
    }
  }
 ...
 */
uint16_t bmi088_set_accel_fifo_filt_data(uint8_t accel_fifo_filter,
                                         struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data = 0, reg_addr;
  ...
  if (rslt == BMI088_OK) {
    if (accel_fifo_filter <= BMI088_MAX_VALUE_FIFO_FILTER) {
      ...
      if (rslt == BMI088_OK) {
        ...
        data = BMI088_SET_BITSLICE(data, BMI088_ACCEL_FIFO_FILT_DATA,
                                   accel_fifo_filter);
 ...
 *  for Accel FIFO data.
 */
uint16_t bmi088_get_fifo_down_accel(uint8_t *fifo_down,
                                    struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data = 0, reg_addr;
...
uint16_t bmi088_set_fifo_down_accel(uint8_t fifo_down, struct bmi088_dev *dev) {
  ...
  if (rslt == BMI088_OK) {
    if (fifo_down <= BMI088_MAX_VALUE_FIFO_DOWN) {
      ...
      if (rslt == BMI088_OK) {
        data =
            BMI088_SET_BITSLICE(data, BMI088_ACCEL_FIFO_FILT_DOWN, fifo_down);
 ...
 */
uint16_t bmi088_get_accel_fifo_length(uint16_t *fifo_length,
                                      struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  ...
  if (rslt == BMI088_OK) {
    ...
    if (rslt == BMI088_OK) {
      ...
      index = BMI088_ONE;
      data[index] =
          BMI088_GET_BITSLICE(data[index], BMI088_FIFO_BYTE_COUNTER_MSB);
 ...
 */
uint16_t bmi088_set_accel_fifo_full_int(struct bmi088_int_cfg *int_config,
                                        struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data = 0, reg_addr;
    ...
    if (rslt == BMI088_OK) {
      ...
      if (rslt == BMI088_OK) {
        ...
        if (rslt == BMI088_OK) {
          ...
          if (int_config->accel_int_channel == BMI088_INT_CHANNEL_1) {
            data = BMI088_SET_BITSLICE(data, BMI088_ACCEL_INT1_FIFO_FULL,
                                       BMI088_ONE);
          } else if (int_config->accel_int_channel == BMI088_INT_CHANNEL_2) {
            data = BMI088_SET_BITSLICE(data, BMI088_ACCEL_INT2_FIFO_FULL,
                                       BMI088_ONE);
          } else {
            /* do nothing */
          }
 ...
 */
uint16_t bmi088_set_accel_fifo_wm_int(struct bmi088_int_cfg *int_config,
                                      struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data = 0, reg_addr;
    ...
    if (rslt == BMI088_OK) {
      ...
      if (rslt == BMI088_OK) {
        ...
        if (rslt == BMI088_OK) {
          ...
          if (int_config->accel_int_channel == BMI088_INT_CHANNEL_1) {
            data = BMI088_SET_BITSLICE(data, BMI088_ACCEL_INT1_FIFO_WM,
                                       BMI088_ONE);
          } else if (int_config->accel_int_channel == BMI088_INT_CHANNEL_2) {
            data = BMI088_SET_BITSLICE(data, BMI088_ACCEL_INT2_FIFO_WM,
                                       BMI088_ONE);
          } else {
            /* do nothing */
          }
...
uint16_t bmi088_get_gyro_fifo_data(struct bmi088_dev *dev) {
  ...
  if (rslt == BMI088_OK) {
    ...
    rslt |= dev->read(dev->gyro_id, reg_addr, dev->gyro_fifo->data,
                      dev->gyro_fifo->length);

    /* Update the data select axes info in fifo_data_enable variable found in
     * device structure */
    rslt |=
        bmi088_get_gyro_fifo_data_sel(&dev->gyro_fifo->fifo_data_enable, dev);
  }
 ...
 *  this API.
 */
uint16_t bmi088_extract_gyro(struct bmi088_sensor_data *gyro_data,
  ...
  if (rslt == BMI088_OK) {
    ...
    for (; data_index < data_read_length;) {
      ...
      rslt |= unpack_gyro_frm(gyro_data, &data_index, &gyro_index,
                              dev->gyro_fifo->fifo_data_enable, dev);
      /* Check for the availability of next
...
/*!
 *  @brief This API gets the data of axes to be stored in the FIFO in the gyro
 * sensor.
 */
uint16_t bmi088_get_gyro_fifo_data_sel(uint8_t *fifo_data_select,
                                       struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data, reg_addr;
  ...
  if (rslt == BMI088_OK) {
    ...
    if (rslt == BMI088_OK) {
      ...
      *fifo_data_select =
          BMI088_GET_BITSLICE(data, BMI088_GYRO_FIFO_DATA_SELECT);
    }
  }
...
/*!
 *  @brief This API sets the data of axes to be stored in the FIFO in the gyro
 * sensor.
 */
uint16_t bmi088_set_gyro_fifo_data_sel(uint8_t fifo_data_select,
                                       struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data, reg_addr;
  ...
  if (rslt == BMI088_OK) {
    if (fifo_data_select <= BMI088_GYRO_Z_DATA) {
      ...
      /* Write the fifo operating mode */
      data = BMI088_SET_BITSLICE(data, BMI088_GYRO_FIFO_DATA_SELECT,
                                 fifo_data_select);
      rslt |= bmi088_set_gyro_regs(reg_addr, &data, BMI088_ONE, dev);
    } else {
      rslt = BMI088_E_OUT_OF_RANGE;
 ...
 */
uint16_t bmi088_get_gyro_fifo_length(uint8_t *fifo_length,
                                     struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data = 0, reg_addr;
 ...
 */
uint16_t bmi088_get_gyro_fifo_overrun(uint8_t *fifo_overrun_status,
                                      struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data = 0, reg_addr;
  ...
  if (rslt == BMI088_OK) {
    ...
    if (rslt == BMI088_OK) {
      ...
      *fifo_overrun_status =
          BMI088_GET_BITSLICE(data, BMI088_GYRO_FIFO_OVERRUN);
    }
  }
 ...
 */
uint16_t bmi088_get_gyro_fifo_tag(uint8_t *fifo_tag_status,
                                  struct bmi088_dev *dev) {
  uint16_t rslt = 0;
  uint8_t data = 0, reg_addr;
...
/*!
 *  @brief This API enables or disables the FIFO full interrupt of the gyro
 * sensor.
 */
uint16_t bmi088_set_gyro_fifo_full_int(struct bmi088_int_cfg *int_config,
                                       struct bmi088_dev *dev,
  ...
  /* Proceed if null check is fine */
  if (rslt == BMI088_OK) {
    /* Read the Interrupt control register and Interrupt configuration register
     * together */
    ...
    if (rslt == BMI088_OK) {
      data[0] =
          BMI088_SET_BITSLICE(data[0], BMI088_GYRO_FIFO_EN, fifo_full_enable);

      /* Interrupt pin or channel 3 */
      if (int_config->gyro_int_channel == BMI088_INT_CHANNEL_3) {
        /* Update data with user configured bmi088_int_cfg structure */
        data[1] = BMI088_SET_BITSLICE(data[1], BMI088_GYRO_INT3_LVL,
                                      int_config->gyro_int_pin_3_cfg.lvl);
        data[1] =
            BMI088_SET_BITSLICE(data[1], BMI088_GYRO_INT3_OD,
      ...
      if (int_config->gyro_int_channel == BMI088_INT_CHANNEL_4) {
        /* Update data with user configured bmi088_int_cfg structure */
        data[1] = BMI088_SET_BITSLICE(data[1], BMI088_GYRO_INT4_LVL,
                                      int_config->gyro_int_pin_4_cfg.lvl);
        data[1] =
            BMI088_SET_BITSLICE(data[1], BMI088_GYRO_INT4_OD,
      ...
      /* Write to Interrupt control register and Interrupt configuration
       * register together */
      ...
      if (rslt == BMI088_OK) {
        ...
        if (int_config->gyro_int_channel == BMI088_INT_CHANNEL_3) {
          /* Map Interrupt pin 3 */
          data[0] =
              BMI088_SET_BITSLICE(data[0], BMI088_GYRO_INT1_FIFO, BMI088_ONE);
        }
        ...
        if (int_config->gyro_int_channel == BMI088_INT_CHANNEL_4) {
          /* Map Interrupt pin 4 */
          data[0] =
              BMI088_SET_BITSLICE(data[0], BMI088_GYRO_INT2_FIFO, BMI088_ONE);
        }
...
/*!
 *  @brief This API enables or disables the FIFO watermark interrupt of the gyro
 * sensor.
 */
uint16_t bmi088_set_gyro_fifo_wm_int(struct bmi088_int_cfg *int_config,
                                     struct bmi088_dev *dev,
                                     uint8_t fifo_wm_enable) {
  uint16_t rslt = 0;
  uint8_t data, reg_addr;
...
static void check_empty_accel_fifo(uint16_t *data_index,
  ...
  if ((*data_index + 2) < dev->accel_fifo->length) {
    ...
    if ((dev->accel_fifo->data[*data_index] == FIFO_MSB_CONFIG_CHECK) &&
        (dev->accel_fifo->data[*data_index + BMI088_ONE] ==
         FIFO_LSB_CONFIG_CHECK)) {
      /* Update the data index as complete*/
...
static void extract_accel_header_mode(struct bmi088_sensor_data *accel_data,
  ...
  for (data_index = dev->accel_fifo->byte_start_idx;
       data_index < dev->accel_fifo->length;) {
    ...
    /* Accel frame */
    case FIFO_HEAD_A: {
      unpack_accel_frm(accel_data, &data_index, &accel_index, frame_header,
                       dev);
    } break;
...
static void unpack_sensortime_frame(uint16_t *data_index,
  ...
  if ((*data_index + BMI088_SENSOR_TIME_LENGTH) > dev->accel_fifo->length) {
    ...
    *data_index = dev->accel_fifo->length;
    ...
    sensor_time_byte3 =
        dev->accel_fifo->data[(*data_index) + BMI088_SENSOR_TIME_MSB_BYTE]
        << BMI088_SIXTEEN;
    sensor_time_byte2 =
        dev->accel_fifo->data[(*data_index) + BMI088_SENSOR_TIME_XLSB_BYTE]
        << BMI088_EIGHT;
    sensor_time_byte1 =
        dev->accel_fifo->data[(*data_index) + BMI088_SENSOR_TIME_LSB_BYTE];
    /* Sensor time */
    dev->accel_fifo->sensor_time =
    ...
    *data_index = dev->accel_fifo->length;
    ...
    if (dropped_frame == ACCEL_FIFO_DROP) {
      *data_index = (*data_index) + BMI088_FIFO_A_LENGTH;
      dev->accel_fifo->dropped_frame_count =
          dev->accel_fifo->dropped_frame_count + 1;
    }
  }
}
...
static uint16_t unpack_gyro_frm(struct bmi088_sensor_data *data, uint16_t *idx,
  ...
  if ((frm == BMI088_GYRO_ALL_INT_DATA) || (frm == BMI088_GYRO_X_DATA) ||
      (frm == BMI088_GYRO_Y_DATA) || (frm == BMI088_GYRO_Z_DATA)) {
    /* Partial read, then skip the data */
...
static void check_empty_gyro_fifo(uint16_t *data_index,
  ...
  if ((*data_index + 2) < dev->gyro_fifo->length) {
    ...
    if ((dev->gyro_fifo->data[*data_index] == FIFO_MSB_CONFIG_CHECK) &&
        (dev->gyro_fifo->data[*data_index + BMI088_ONE] ==
         FIFO_LSB_CONFIG_CHECK)) {
      /* Update the data index as complete */
