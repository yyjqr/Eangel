...
/**
 @file    main.c
  @brief  Imu accelerometer gyroscope
 6轴数据100Hz采集获取，温度的读取，保存在mnt下设备号目录文件中
...
int configIMU(struct bmi088_dev *dev) {
  int ret = -1, rslt = -1;
  int8_t self_rslt;
  rslt = bmi088_accel_switch_control(
      dev, BMI088_ACCEL_POWER_ENABLE); // 使能ACC SENSOR！！
  if (rslt == 0) {
...
int main(int argc, char **argv) {
  ...
  imu0.gyro_cfg.odr =
      BMI088_GYRO_BW_47_ODR_400_HZ; // add  setup 1125
                                    // BMI088_GYRO_BW_32_ODR_100_HZ---->BMI088_GYRO_BW_47_ODR_400_HZ
                                    // 1203---> BMI088_GYRO_BW_116_ODR_1000_HZ
  ...
  /*  if (pthread_create(&save_temperature_thread_t, NULL,
  save_temperature_thread, NULL))
  {
...
void *save_imu_thread() {
  ...
  if (0 == ret) {
    ...
    log_error("failed to get_device_number from %s, ret:%d ", STAR_OS_CONFIG,
              ret);
    return ret;
  }
  sprintf(imu_path, "%s/%d%02d%02d_%02d%02d%02d_imu.txt", sn,
  ...
  while (1) {
    ...
    if (imu_fp != NULL) {
      ...
      // if (rdyByte == 0x80)
      {
        ...
        // uint16_t *accel_length, const struct bmi088_dev *dev)
        bmi088_get_accel_data(&acc_val, &imu0); //
        acc_x_in_mg = acc_val.x / 32768.0 * 1000 *
                      3; // 3g 量程     0x01   2^(0+1)*1.5=3 新版已解决0915
        ...
        clock_gettime(CLOCK_REALTIME, &tv); // CLOCK_MONOTONIC
                                            // --->CLOCK_REALTIME
        double imu_utc_time =
            tv.tv_sec +
            tv.tv_nsec /
                1000000000.0; // REVISE 0130  clock_gettime(CLOCK_MONOTONIC,
                              // ...) provides nanosecond resolution, is
                              // monotonic. I believe the 'seconds' and
                              // 'nanoseconds' are stored separately, each in
                              // 32-bit counters.
                              //  printf("board system is %lf\n",imu_utc_time);
                              //  //test
                              //  //1541472902.070,$IMU,CHIP,0,RESULT,IMU,ax,0.069580,ay,0.060059,az,1.026611,gx,0.610352,gy,-0.122070,gz,1.327515,*FF
        fprintf(imu_fp,
                "%.3lf,$IMU,BMI,0,RESULT,IMU,ax,%f,ay,%f,az,%f,gx,%f,gy,%f,gz,%"
                "f,*FF\n",
                imu_utc_time,

                acc_x_in_mg, acc_y_in_mg, acc_z_in_mg, gyro_x_in_deg,
                gyro_y_in_deg, gyro_z_in_deg);
      ...
      if ((cur_tp.tv_sec - begin_write_tp.tv_sec) >
          60 * 60) { // 每次录制60分钟 0122
        printf("cur_tp.tv_sec is %ld,  begin_write_tp.tv_sec is %ld  /n",
               cur_tp.tv_sec, begin_write_tp.tv_sec);
