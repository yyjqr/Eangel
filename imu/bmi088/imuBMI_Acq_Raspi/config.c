...
int getConfigEntry(char *grpName, char *entryName, char *entryBuf,
  ...
  if (entryName != NULL && entryBuf != NULL) {
    ...
    if (len_entry_name >= MAX_LEN_ENTRY_NAME ||
        entryBufSize > MAX_LEN_ENTRY_VALUE)
      return -22;
  } else {
    return -23;
  }
  ...
  if (setCurConfigGroup(grpName) == 0) {
    ...
    if (curConfigGroup &&
        !list_empty_careful(&curConfigGroup->listConfigEntry.listEntry)) {

      if (curConfigEntry != NULL) {
        if (strcmp(curConfigEntry->name, entryName) == 0) {

          if (curConfigEntry->valueLen < entryBufSize) {
            // memcpy(entryBuf, curConfigEntry->value,
            // curConfigEntry->valueLen);
      ...
      list_for_each_safe(pos_entry, n_entry,
                         &curConfigGroup->listConfigEntry.listEntry) {
    ...
    if (ptrValue != NULL) // add a new entry
    {
      ...
      if ((ptrValue - ptr) < MAX_LEN_ENTRY_NAME) {
        ...
        if ((*ptrValue == '\"') || (*ptrValue == '\'')) {
          ...
          do {
            if ((*ptrValue == '\0') ||
                (len >= MAX_LEN_ENTRY_VALUE)) { // Exception handle
              // opBuf[0] = '\0';
              // break;
              // xcpPtr->error(E_CONFIG_ERROR, SF_CFGBASE, "Missing delimitation
              // char \' or \"."); return NULL;
              continue;
            }
