...
// UINT32 ssp_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data, uint16_t
// len)
int8_t user_spi_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data,
                     uint16_t len) {
  ...
  unsigned int spi_num = 0, csn = 1,
               cur_reg_addr; // dev_addr = 0, reg_addr = 0, cur_reg_addr = 0;
  unsigned int num_reg = 1, dev_width = 1, reg_width = 1, data_width = 1,
               reg_order = 1, data_order = 1;
  uint8_t tempValue[6];

  num_reg = len; // add  data_width=len ----》num_reg=len
                 // 读取长度，即读取的寄存器个数  1123！！！
...
dev_width, reg_width, data_width, reg_order, data_order);*/
  ...
  for (cur_reg_addr = reg_addr, i = 0; cur_reg_addr < reg_addr + num_reg;
       cur_reg_addr++, i++) {
    index = 0; // test
    ...
    *data =
        tempValue[0]; // add  数组的首地址返回给单字节读取  或多字节数据读取。
...
int8_t user_spi_write(uint8_t id, uint8_t reg_addr, uint8_t *data,
                      uint16_t len) {
  ...
  unsigned int dev_width = 1, reg_width = 1, data_width = 1, reg_order = 1,
               data_order = 1;

  /*printf("spi_num:%u, csn:%u\n"
                  "dev_addr:0x%04x, reg_addr:0x%04x, data:0x%04x, "
                  "dev_width:%d, reg_width:%d, data_width:%d, reg_order: %d,
     data_order: %d\n", spi_num, csn, dev_addr, reg_addr, data, dev_width,
...
int8_t user_spi_readACC(uint8_t dev_addr, uint8_t reg_addr, uint8_t *data,
                        uint16_t len) {
  int fd;
  int ret = -1;
  int tmp = 0;
...
int8_t user_spi_writeACC(uint8_t id, uint8_t reg_addr, uint8_t *data,
                         uint16_t len) {
  int fd;
  int ret = -1;
  int tmp = 0;
