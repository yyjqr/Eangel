...
# 反反爬：magic 模式下 crawl4ai 自动注入随机鼠标移动、canvas噪声、navigator 伪装等
_STEALTH_HEADERS = {
    "User-Agent": (
...
def send_alert_email(content_list):
    ...
    subject = f"🚀 Jetson AI ISP SOC分析 ({datetime.now().strftime('%m-%d')})"
...
if __name__ == "__main__":
    config_path = os.path.join(os.path.dirname(__file__), "urls_config_camera_isp_soc.json")
