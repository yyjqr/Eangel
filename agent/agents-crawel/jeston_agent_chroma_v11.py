...
_CRAWL_TIMEOUT = 90000
_CRAWL_RETRIES = 2
_CRAWL_DELAY = 3
_ANALYSIS_SOURCE_MAX_CHARS = int(RANK_CONFIG.get("ANALYSIS_SOURCE_MAX_CHARS", 8000))
# 每个来源最多提取条目数；调大可获得更多新闻（原值 4）
_ANALYSIS_MAX_ITEMS_PER_SOURCE = int(RANK_CONFIG.get("ANALYSIS_MAX_ITEMS_PER_SOURCE", 6))
...
# 语义重复阈值：distance < 此值视为重复；调大可放行更多相似新闻（原值 0.35）
_DUPLICATE_DISTANCE_THRESHOLD = float(RANK_CONFIG.get("DUPLICATE_DISTANCE_THRESHOLD", 0.40))
# 邮件告警阈值：score >= 此值发送邮件；调小可发送更多预警（原值 3.0）
_ALERT_SCORE_THRESHOLD = float(RANK_CONFIG.get("ALERT_SCORE_THRESHOLD", 2.5))
_CURRENT_DATE = datetime.now().strftime("%Y-%m-%d")
_CURRENT_YEAR = datetime.now().year

# 邮件发送次数限制规则（从配置文件加载）
_EMAIL_SEND_RULES = RANK_CONFIG.get("EMAIL_SEND_RULES", {
    "age_2y_max_sends": 1,
    "age_1y_max_sends": 2,
    "default_max_sends": 999,
})
_EMAIL_SEND_LOG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "email_send_log.json")

# 已知加载慢/JS渲染重的域名：使用更长延迟 + 降级重试策略
_SLOW_DOMAINS: tuple = tuple(RANK_CONFIG.get("SLOW_DOMAINS", [
    "wallstreetcn.com",
    "makemagazine.com",
    "wired.com",
    "theverge.com",
...
# 不使用 ONNX 内容评分过滤器，避免 Jetson ARM64 上的 onnxruntime 兼容问题
# 直接用纯规则的 DefaultMarkdownGenerator，不传入 content_filter
...
# ISP 知识库并行写入
_isp_client = chromadb.PersistentClient(path="./my_chroma_db")
_isp_collection = _isp_client.get_or_create_collection(name="isp_sdk_docs")
_ISP_CATEGORIES = ("芯片/AI", "科技")
...
def _load_email_send_log() -> dict:
    """加载邮件发送日志（记录每篇文章的历史发送次数）。"""
    try:
        with open(_EMAIL_SEND_LOG_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        return {}


def _save_email_send_log(log: dict) -> None:
    """持久化保存邮件发送日志。"""
    with open(_EMAIL_SEND_LOG_PATH, "w", encoding="utf-8") as f:
        json.dump(log, f, ensure_ascii=False, indent=2)


def _parse_publish_date(publish_date: str):
    """将字符串日期解析为 datetime，支持 YYYY-MM-DD / YYYY-MM / YYYY 格式。"""
    for fmt in ("%Y-%m-%d", "%Y-%m", "%Y"):
        try:
            return datetime.strptime(publish_date.strip(), fmt)
        except (ValueError, AttributeError):
            continue
    return None


def _get_max_sends(publish_date: str) -> int:
    """根据文章发布日期与配置规则，返回允许的最大发送次数。"""
    if not publish_date:
        return int(_EMAIL_SEND_RULES.get("default_max_sends", 999))
    dt = _parse_publish_date(publish_date)
    if dt is None:
        return int(_EMAIL_SEND_RULES.get("default_max_sends", 999))
    age_days = (datetime.now() - dt).days
    if age_days >= 730:   # 2 年前及更早
        return int(_EMAIL_SEND_RULES.get("age_2y_max_sends", 1))
    if age_days >= 365:   # 1~2 年前
        return int(_EMAIL_SEND_RULES.get("age_1y_max_sends", 2))
    return int(_EMAIL_SEND_RULES.get("default_max_sends", 999))


def _can_send_alert(article_id: str, publish_date: str, send_log: dict) -> bool:
    """判断该文章是否还可以触发邮件告警（未超过最大发送次数）。"""
    max_sends = _get_max_sends(publish_date)
    current_count = send_log.get(article_id, {}).get("send_count", 0)
    return current_count < max_sends


def _record_send(article_id: str, title: str, publish_date: str, send_log: dict) -> None:
    """在内存中的 send_log 里记录一次发送（调用方负责持久化）。"""
    if article_id not in send_log:
        send_log[article_id] = {
            "title": title,
            "publish_date": publish_date,
            "send_count": 0,
        }
    send_log[article_id]["send_count"] += 1
    send_log[article_id]["last_sent"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def _make_run_config(url: str, *, fallback: bool = False) -> CrawlerRunConfig:
    """根据 URL 特征生成爬取配置。
    - 慢站点/JS重站点: 增大 delay_before_return_html
    - fallback 模式: 放弃 magic 模式，用最简单配置重试（应对反爬误杀）
    - 不使用 ONNX 内容过滤器，避免 Jetson ARM64 兼容问题
    """
    is_slow = any(d in url for d in _SLOW_DOMAINS)
    return CrawlerRunConfig(
        cache_mode=CacheMode.BYPASS,
        markdown_generator=_MD_GENERATOR,  # 不含 ONNX 内容过滤，解决 ARM64 onnxruntime 报错
        page_timeout=_CRAWL_TIMEOUT,
        wait_until="domcontentloaded",
        delay_before_return_html=3.0 if is_slow else 0.8,
        remove_overlay_elements=True,   # 自动关闭 Cookie/订阅弹窗，改善内容质量
        remove_consent_popups=True,
        magic=not fallback,             # fallback 时关闭 magic，避免与某些站点冲突
        simulate_user=not fallback,
        override_navigator=not fallback,
...
async def _fetch_url(crawler, url: str) -> str:
    ...
    第 1 次用完整 magic 配置；失败后用降级配置重试，避免 magic 模式触发反爬。
    """
    for attempt in range(1, _CRAWL_RETRIES + 1):
        fallback = attempt > 1   # 第 2 次及以后降级为简单模式
        config = _make_run_config(url, fallback=fallback)
        try:
            result = await crawler.arun(url=url, config=config)
            if result.success:
                # 内容质量检查：过于简短说明 JS 渲染未完成，但仍返回（由 LLM 判断）
                visible_len = len((result.markdown or "").strip())
                if visible_len < 200 and not fallback:
                    print(f"[WARN] {url} 内容极少({visible_len}字)，可能 JS 渲染未完成")
                print(f"[OK] {url}")
            ...
            if "Timeout" in err_msg or "timeout" in err_msg:
                print(f"[WARN] attempt {attempt}/{_CRAWL_RETRIES} timeout for {url}")
            else:
...
def storage_node(state: AgentState):
    ...
    try:
        ...
        print(f"[WARN] MySQL 连接失败，本次仅写入 Chroma: {db_err}")
        conn = None
        cursor = None
        db_available = False
    ...
    for event in events:
        ...
        if not is_duplicate:
            article_id = hashlib.md5((title + summary).encode()).hexdigest()
            collection.add(
                documents=[summary],
                metadatas=[{
                    "title": title,
            ...
            # 双写 ISP 知识库（仅 ISP/芯片相关分类）
            if category.strip() in _ISP_CATEGORIES:
                isp_id = "isp-" + article_id
                isp_search = _isp_collection.query(query_texts=[summary], n_results=1)
                isp_dists = isp_search.get("distances") or []
                isp_dup = False
                if isp_dists and isinstance(isp_dists[0], list) and isp_dists[0]:
                    if isp_dists[0][0] < _DUPLICATE_DISTANCE_THRESHOLD:
                        isp_dup = True
                if not isp_dup:
                    _isp_collection.add(
                        documents=[summary],
            ...
            if should_alert:
                if _can_send_alert(article_id, publish_date, send_log):
                    date_label = publish_date if publish_date else "未知"
                    alert_text = (
                        f"🚨 【高价值分析 ({score})】\n"
                        f"标题: {title}\n"
                    ...
                    _record_send(article_id, title, publish_date, send_log)
                else:
                    max_s = _get_max_sends(publish_date)
                    cur_s = send_log.get(article_id, {}).get("send_count", 0)
                    print(f"⛔ 已达发送上限({cur_s}/{max_s})，跳过告警: {title[:30]}...")

    if db_available:
        cursor.close()
        conn.close()

    _save_email_send_log(send_log)
    return {
...
def send_alert_email(content_list):
    ...
    subject = f"🚀 Jetson AI 自动价值分析 ({datetime.now().strftime('%m-%d')})"
...
if __name__ == "__main__":
    ...
    if final_state.get("alert_queue"):
        ...
        print(f"✅ 任务完成，发送了 {len(final_state['alert_queue'])} 条高价值分析。")
    else:
        print("本次无高价值分析。")
