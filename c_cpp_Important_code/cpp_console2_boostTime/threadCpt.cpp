...
// 线程要做的事情就写在这个线程函数中
void GetSumT(vector<int>::iterator first, vector<int>::iterator last,
             long int &result) {
  result = accumulate(first, last, 0); // 调用C++标准库算法
}

// void GetSumT(vector<long int>::iterator first,vector<long  int>::iterator
// last,long  int &result)
// {
//     result = accumulate(first,last,0); //调用C++标准库算法
// }
...
int main() // 主线程
{
  ...
  thread first(GetSumT, largeArrays.begin(), largeArrays.begin() + 200,
               std::ref(result1)); // 子线程1
  cout << "result1:" << result1 << endl;
  thread second(GetSumT, largeArrays.begin() + 2000, largeArrays.begin() + 4000,
                std::ref(result2)); // 子线程2
  ...
  thread fifth(GetSumT, largeArrays.begin() + 80000000, largeArrays.end(),
               std::ref(result5)); // 子线程5
  ...
  int resultSum =
      result1 + result2 + result3 + result4 + result5; // 汇总各个子线程的结果
