...
int main(int argc, char **argv) {
  ...
  cout << "st_robot id:" << st_robot.carID
       << "st_robot veloctity:" << st_robot.veloctity << endl;
  ...
  cout << "After inital,st_robot id:" << p_st_robot->carID
       << "st_robot veloctity:" << p_st_robot->veloctity << endl;

  boost::posix_time::ptime pTime =
      boost::posix_time::microsec_clock::local_time();
  //    boost::posix_time::time_duration now_time_of_day =
  //    boost::posix::microsec_clock::local_time().;
  cout << "start time:" << pTime << endl;
  std::string strTimeOfDay = boost::posix_time::to_simple_string(
      pTime.time_of_day()); // 当前时间：15:03:55
  ...
  cout << "uint32_t timestamp:" << timestamp32 << " \n 32value:" << value
       << "value64:" << value64 << endl;

  uint64_t timestamp =
      boost::chrono::duration_cast<boost::chrono::milliseconds>(
          boost::chrono::steady_clock::now().time_since_epoch())
          .count();
  cout << "uint64_t timestamp:" << timestamp << "test 32value :" << valueMax
       << endl;
