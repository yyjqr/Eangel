/****定时拍摄10分钟的图片，并以当前时间+字符的形式命名***by
 * YJ20170908----》201907 --->202005 */
...
int main(int argc, char **argv) {
  ...
  pCapture.open(-1); // 从摄像头读入视频 0表示从摄像头读入 -1 表示任意摄像头
                     // 2020 double rate=25.0;//fps

  pCapture.set(CAP_PROP_FRAME_WIDTH, 1280);
  pCapture.set(CAP_PROP_FRAME_HEIGHT, 960);

  string str[2000] = {
      "cam0"}; // 数组设置过小，导致只能拍摄100张图片，现已修改为2000
  ...
  for (unsigned int i = 0; i < timeDuration * 42;
       i++) //*60/5*4  (20180807yj)------------>*60/5.5*4 (1121)
  {
    pCapture >> frame;
    if (frame.empty()) {
    ...
    waitKey(900); // 延时5s
    j = i % 4;
    if (i == 10) // 初始保存一张，一般每次开机拍摄能快速获取最新的照片
                 // 前面几张比较暗，获取的第10次的，第三张左右的图片20181012
    {
      t = time(
    ...
    if (j == 0) {
      ...
      waitKey(
          1500); // 延时2.5s---------------->1.5s
                 // （20181015减少拍摄时间，周期5.5s，但系统实际可能达6s，这样保证拍摄时长10分钟，并间隔较小！！！
    }
  }
