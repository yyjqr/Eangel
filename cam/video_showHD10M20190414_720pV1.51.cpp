...
int main() {
  ...
  VideoCapture videoCapturer(
      0); // 如果是笔记本，0打开的是自带的摄像头，1 打开外接的相机
  /**
  ...
  if (videoCapturer.isOpened()) {
    ...
    cout << "video info"
         ...
         << ", FPS: " << videoCapturer.get(CV_CAP_PROP_FPS) << ", FrameCount: "
         << " "
  ...
  VideoWriter writer(
      pVideoFileName, CV_FOURCC('M', 'P', '4', '2'),
      videoCapturer.get(CV_CAP_PROP_FPS),
      Size(videoCapturer.get(CV_CAP_PROP_FRAME_WIDTH),
           videoCapturer.get(
               CV_CAP_PROP_FRAME_HEIGHT))); // AVI 0901   avi格式 MJPG编码
